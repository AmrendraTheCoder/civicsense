...
import {
  LayoutDashboardIcon, FilePlusIcon, MicIcon, BarChart3Icon,
  MapPinIcon, ShieldIcon, HelpCircleIcon, TrendingUpIcon,
  ChevronRightIcon, XIcon,
} from 'lucide-react';
import { useCivic } from '../context/CivicContext';

const NAV_ITEMS = [
  { id: 'dashboard',            label: 'Dashboard',           icon: LayoutDashboardIcon, badge: null,  description: 'Overview & analytics'   },
  { id: 'file-complaint',       label: 'File Complaint',       icon: FilePlusIcon,        badge: 'New', description: 'Submit an issue'         },
  { id: 'voice-assistant',      label: 'Voice Assistant',      icon: MicIcon,             badge: 'AI',  description: 'Speak your complaint'    },
  { id: 'predictive-analytics', label: 'Predictive Analytics', icon: BarChart3Icon,       badge: null,  description: 'AI-powered insights'     },
];
...
function DesktopSidebar({ activeView, setActiveView, sidebarOpen, setSidebarOpen, stats }) {
  return (
    <aside
      id="main-sidebar"
      className={`
        hidden md:flex fixed top-16 left-0 h-[calc(100vh-4rem)] z-40 flex-col
                ...
                >
                  ...
                  {sidebarOpen && (
                    <>
                      ...
                      {isActive && <ChevronRightIcon size={14} className="text-blue-400 shrink-0" />}
                    </>
                  )}
      ...
      {NAV_ITEMS.map(({ id, label, icon: Icon, badge }) => {
        ...
        return (
          ...
          >
            {/* Active glow */}
            {isActive && (
              <div className="absolute top-1 left-1/2 -translate-x-1/2 w-8 h-8 rounded-full blur-md opacity-40"
            ...
            <div className={`relative w-8 h-8 rounded-xl flex items-center justify-center transition-all duration-200 ${
              ...
              style={isActive ? { background: 'rgba(59,130,246,0.18)' } : {}}>
              <Icon size={18} />
            ...
            {/* Label */}
            <span className={`text-[9px] font-semibold transition-colors leading-none ${
...
export default function Sidebar() {
  const { activeView, setActiveView, sidebarOpen, setSidebarOpen, stats } = useCivic();
